...
Updates based on tt-dfd PDF (pp. 26–27 programming sequence):
  - Added activate() for EnableCla (bit6) with readback (post-config, pre-EAP enable).
  - Added set_mux_sel() for CDbgSignalMuxSel (~0x3000 offset; patch per MMR map).
  - Enhanced reset_cla() to include mux reset to 0 and timestamp sync=0.
  - Added retry on slverr in apb_write/read (up to 3 attempts).
"""
...
MUX_SEL_BASE  = 0x3000          # CDbgSignalMuxSel (patch from MMR map if needed)
...
# [6]     EnableCla                  — master CLA enable (active=1 readback per p.26)
...
CTRL_CLA_EN_BIT          = 6    # EnableCla (active)
...
class CLADriver:
    ...
    def build_eap(evt0=EVT_DISABLE, evt1=EVT_ALWAYS_ON, evt2=EVT_ALWAYS_ON,
                  ...
                  dest_node=0,
                  cact0=0, cact0_en=False,
                  cact1=0, cact1_en=False):
        """
    ...
    async def enable_eap(self):
        """Set EnableEap (bit 5) in CTRL_STATUS to activate programmed EAPs."""
        await self.apb.read_modify_write(
            CLA_REG["CTRL_STATUS"],
            set_bits=(1 << CTRL_EAP_EN_BIT)
        )
    ...
    async def activate(self):
        """
        Set EnableCla (bit 6) in CTRL_STATUS for master CLA activation
        with readback (per PDF p.26 programming sequence).
        """
        await self.apb.read_modify_write(
            CLA_REG["CTRL_STATUS"],
        ...
        if not ((val >> CTRL_CLA_EN_BIT) & 1):
            raise ValueError("CLA activation failed: EnableCla bit not set after write")
    ...
    async def set_mux_sel(self, lane=0):
        """
        Program CDbgSignalMuxSel to select debug bus lane (e.g., hw0=0).
        Offset patched from MMR map (default ~0x3000 per CR_Registers.html).
        """
        await self.apb.write(MUX_SEL_BASE, lane & 0xFFFF_FFFF)
    ...
    async def reset_cla(self):
        """
        Full software reset sequence (enhanced per PDF p.26–27):
          1. Set mux to 0 (default lane)
          2. Disable EAP
          3. Clear all four counters
          4. Clear EAP status
          5. Deassert time-match
          6. Sync timestamps (write 0 to CDbgClaTimestampsync)
        """
        await self.set_mux_sel(0)
        await self.disable_eap()
        for i in range(4):
            await self.clear_counter(i)
        await self.clear_eap_status()
        await self.set_time_match(0)
        await self.apb.write(CLA_REG["TIMESTAMP_SYNC"], 0)
