...
async def L2_CLA_001_event_disable(dut):
    ...
    await cla.program_eap(0, 0,
        evt0=EVT_DISABLE, udf=UDF_ALWAYS, act0=ACT_START_TRACE)
    await cla.enable_eap()

    # Drive every possible bus pattern — action must never assert
...
async def L2_CLA_003_udf_all_8_patterns(dut):
    """
    L2_CLA_003 – UDF truth table: exhaustively verify UDF_AND_ALL, UDF_OR_ANY,
    UDF_ALWAYS, UDF_E0_ONLY, UDF_E1_ONLY, UDF_E2_ONLY, UDF_E1_AND_E0, UDF=0x00.
    """
    log.info("=== L2_CLA_003: UDF All Patterns ===")

    # Each tuple: (udf, evt0, evt1, evt2, should_fire)
    # E2 = ALWAYS_ON (always 1). So:
    #   UDF_AND_ALL (0x80): fires when E0=1, E1=1, E2=1  → need E0+E1 match
    #   UDF_OR_ANY  (0xFE): fires when any of E0,E1,E2 non-zero → fires on E2 alone
    #   UDF_ALWAYS  (0xFF): always fires
    #   UDF_E0_ONLY (0xAA): fires when E0=1
    #   UDF_E1_ONLY (0xCC): fires when E1=1
    #   UDF_E2_ONLY (0xF0): fires when E2=1 → fires always (E2=ALWAYS_ON)
    #   UDF_E1_AND_E0(0x88): fires when E0=1 AND E1=1
    #   UDF=0x00:   never fires
    ...
    test_cases = [
        # (name, udf, bus_val, e0_evt, e1_evt, expect_fire)
        ...
        ("OR_ANY none",     UDF_OR_ANY,    0x0000, EVT_MATCH1_POS, EVT_MATCH2_POS, True),  # E2=ALWAYS
        ("ALWAYS noop",     UDF_ALWAYS,    0x0000, EVT_DISABLE,    EVT_DISABLE,    True),
        ...
        ("E1_AND_E0",       UDF_E1_AND_E0, 0xCDAB, EVT_MATCH1_POS, EVT_MATCH2_POS, True),
        ("UDF=0 never",     0x00,          0xCDAB, EVT_MATCH1_POS, EVT_MATCH2_POS, False),
    ]

    for name, udf, bus_val, evt0, evt1, expect in test_cases:
        apb, cla, _, _ = await _setup(dut)
        ...
        assert fired == expect, \
            f"UDF test '{name}': expected fire={expect}, got fire={fired} " \
            f"(udf=0x{udf:02X}, bus=0x{bus_val:04X})"
        log.info(f"  '{name}' → fire={fired} ✓")

    log.info("L2_CLA_003 PASSED")
...
async def L2_CLA_005_node_stay_when_no_eap_fires(dut):
    ...
    Program Node0 EAP0 to fire only on a match, then verify node stays at 0
    when the match is not presented.
    """
...
async def L2_CLA_006_four_node_chain(dut):
    ...
    Verifies all four node registers are live and transitions are latched correctly.
    """
    log.info("=== L2_CLA_006: Four-Node Chain ===")
    apb, cla, _, _ = await _setup(dut)

    patterns = [0x0001, 0x0002, 0x0004, 0x0008]

    for i in range(4):
        await cla.set_mask_match(i, 0x000F, patterns[i])

    # Node N on match(N) → go to Node (N+1)%4
    # Node 0 uses mask/match set 0, Node 1 uses set 1, etc.
    # The match set index maps to the event: set 0 → MATCH1_POS, set 1 → MATCH2_POS
    # (Only sets 0 and 1 have dedicated event codes; for sets 2/3 we route via set0 mask)
    # Simplification: use set 0 for all nodes with a different mask pattern each time
    events = [EVT_MATCH1_POS, EVT_MATCH2_POS, EVT_MATCH1_POS, EVT_MATCH2_POS]

    # Re-program sets to isolate each node
    for i in range(4):
        dest = (i + 1) % 4
        evt  = events[i]
        node = i
        await cla.set_mask_match(i % 2, 0x000F, patterns[i])
        await cla.program_eap(node, 0,
            evt0=evt, udf=UDF_E0_ONLY, act0=ACT_NULL, dest_node=dest)

    await cla.enable_eap()

    expected_transitions = [(0, patterns[0]), (1, patterns[1]),
                             (2, patterns[2]), (3, patterns[3])]

    for from_node, trigger in expected_transitions:
        current = await cla.get_current_node()
        assert_eq(current, from_node,
                  f"Expected Node{from_node} before trigger 0x{trigger:04X}")
    ...
    assert_eq(final, 0, "Chain should wrap back to Node0 after Node3 transitions")
...
async def L2_CLA_010_counter_clear_action(dut):
    """
    L2_CLA_010 – ACT_CLR_CTR0 resets counter so that a subsequent match event
    does NOT re-fire until the counter accumulates again.
    EAP0: ALWAYS_ON → AutoIncr; EAP1: MATCH → ClrCtr0; EAP2: Ctr0==TARGET → Halt.
    Drive match before counter reaches target → counter resets → halt delayed.
    """
    ...
    await cla.program_eap(0, 0,
        evt0=EVT_ALWAYS_ON, udf=UDF_E0_ONLY, act0=ACT_AUTO_INCR_CTR0)
    await cla.program_eap(0, 1,
        evt0=EVT_MATCH1_POS, udf=UDF_E0_ONLY, act0=ACT_CLR_CTR0)
    await cla.program_eap(0, 2,
        evt0=EVT_CTR0_MATCH, udf=UDF_E0_ONLY, act0=ACT_CLOCK_HALT)
    await cla.enable_eap()

    # Let counter run halfway, then clear it
    await ClockCycles(dut.clk, TARGET // 2)
    await drive_debug_bus(dut, 0x00AA)   # triggers CLR_CTR0
    await _settle(dut, 3)
    await drive_debug_bus(dut, 0x0000)

    # Halt should not have fired yet (counter was cleared)
    halt_mid = int(dut.external_action_halt_clock_out.value)
    assert halt_mid == 0, "Halt fired before counter reached target after clear"

    # Now let the counter accumulate a full TARGET cycles — halt should fire
    await ClockCycles(dut.clk, TARGET + 10)
    halt_final = int(dut.external_action_halt_clock_out.value)
    assert halt_final == 1, "Halt should fire after counter re-accumulates to target"
...
async def L2_CLA_012_snapshot_captures_bus_on_trigger(dut):
    ...
    Drive 0xABCD, trigger EAP, read snapshot and confirm [15:0] == 0xABCD.
    """
    log.info("=== L2_CLA_012: Snapshot Register Content ===")
    apb, cla, _, _ = await _setup(dut)

    BUS_VAL = 0x00CD
    await cla.set_mask_match(0, 0x00FF, BUS_VAL)
    await cla.program_eap(0, 0,
        evt0=EVT_MATCH1_POS, udf=UDF_E0_ONLY, act0=ACT_NULL)
    await cla.enable_eap()

    # Set upper byte too via hw1
    dut.hw1.value = 0xAB
    await drive_debug_bus(dut, BUS_VAL)
    await _settle(dut, 5)

    snap = await cla.read_snapshot(0, 0)
    log.info(f"L2_CLA_012: snapshot = 0x{snap:08X}")
    # Lower 8 bits must match BUS_VAL
    assert (snap & 0xFF) == BUS_VAL, \
        f"Snapshot byte0: expected 0x{BUS_VAL:02X}, got 0x{snap & 0xFF:02X}"
...
async def L2_DST_004_all_8_lanes_data_integrity(dut):
    ...
    assert delta >= 10, \
        f"Expected ≥10 bytes for all-lanes change, got {delta} bytes"
