...
# DST bit positions
# NOTE: trDstEmpty / trTeEmpty at bit 3 is NOT confirmed by simulation.
# Observed DST_CONTROL value 0x03000065 after disable shows bits 0,2,5,6 set.
# Bits 24-25 = implementation/version RO fields.
# Empty polling now uses scan_for_empty() which discovers the bit position
# at runtime rather than hard-coding bit 3.
DST_CTRL_ACTIVE_BIT       = 0
DST_CTRL_ENABLE_BIT       = 1
DST_CTRL_INST_TRACING_BIT = 2
DST_CTRL_EMPTY_BIT        = 3   # may differ — use wait_empty_safe() instead of poll_field
DST_RAM_CTRL_ACTIVE_BIT   = 0
DST_RAM_CTRL_ENABLE_BIT   = 1
DST_RAM_CTRL_EMPTY_BIT    = 2   # may differ — same caveat
...
# ─────────────────────────────────────────────────────────────────────────────
# FRAME LENGTH — CRITICAL CONFIGURATION
#
# From dfd_trace_sink.sv:
#   trntrFrameLength_ANY[9:0] = {TrCsrTrramimpl.Trramvendorframelength[3:0], 6'b0}
#   trdstFrameLength_ANY[9:0] = {TrCsrTrdstramimpl.Trdstramvendorframelength[3:0], 6'b0}
#
# When Trramvendorframelength = 0 (reset default):
#   FrameLength = 0 → WP pointer increments by 0 → WP never changes → no SRAM data
#
# The WP only updates when a complete frame is written. With FrameLength=0
# the frame advance is 0 bytes per frame, so the write pointer never moves
# and no data appears readable.
#
# Fix: program Trramvendorframelength = 1 → FrameLength = 64 bytes (minimum).
# This field lives in the IMPL register (TrRamImpl / TrDstRamImpl).
# From spec p.26: "Program Vendorfields (Frame length – If needed)".
#
# IMPL register Trramvendorframelength is at bits [3:0] within the IMPL word.
# Address: RAM_IMPL = 0x5004 (NTR), DST_RAM_IMPL = 0x6004 (DST).
# ─────────────────────────────────────────────────────────────────────────────
VENDOR_FRAME_LEN_64B  = 0x1   # bits[3:0] of IMPL = 1 → 64-byte frames
VENDOR_FRAME_LEN_128B = 0x2   # bits[3:0] of IMPL = 2 → 128-byte frames
VENDOR_FRAME_LEN_256B = 0x4   # bits[3:0] of IMPL = 4 → 256-byte frames

# ─────────────────────────────────────────────────────────────────────────────
# APB READ LATENCY FOR RAM_DATA
#
# From dfd_apb2mmr.sv comment:
...
#
# Any read of RAM_DATA (0x5040 / 0x6040) requires the APB to be issued,
# then 3 clock cycles must elapse before re-reading the register returns
# the valid data. The APBMaster.read() method already adds 1 APB cycle
# (setup + enable + pready) so we need at least 2 extra ClockCycles after
# each read transaction.
# ─────────────────────────────────────────────────────────────────────────────
RAM_DATA_READ_LATENCY_CYCLES = 3   # additional cycles to wait after APB write to RP
...
class APBMaster:
    ...
    def __init__(self, dut, clk_period_ns=CLK_PERIOD_NS):
        ...
        # Expose dut directly so DSTDriver/NTraceDriver can call ClockCycles
        self.dut = dut
...
class DSTDriver:
    ...
    async def configure_sram(self, start=0, limit=0x7FFF, stop_on_wrap=True,
                              frame_len=VENDOR_FRAME_LEN_64B):
        """
        Configure the DST RAM sink.

        CRITICAL FIX: programs Trdstramvendorframelength in the DST_RAM_IMPL
        register BEFORE enabling the RAM.

        From dfd_trace_sink.sv:
          trdstFrameLength_ANY[9:0] = {TrCsrTrdstramimpl.Trdstramvendorframelength[3:0], 6'b0}
        When this field = 0 (reset default), FrameLength = 0, the write pointer
        increments by 0 per frame, and WP never advances — no SRAM data is written.

        frame_len: bits[3:0] of the IMPL register.
          1 → 64-byte frames (minimum, default)
          2 → 128-byte frames
          4 → 256-byte frames
        """
        # Step 1: Activate RAM
        ...
        # Step 2: CRITICAL — program vendor frame length in IMPL register
        # Trramvendorframelength is at bits [3:0] of the IMPL register.
        # Must be non-zero before enabling RAM writes.
        await self.apb.write(DST_REG["DST_RAM_IMPL"], frame_len & 0xF)

        # Step 3: Mode = SRAM (bit 0 of mode field = 0, already default)
        if stop_on_wrap:
            await self.apb.read_modify_write(
                DST_REG["DST_RAM_CONTROL"], set_bits=(1 << DST_RAM_CTRL_STOP_ON_WRAP))

        # Step 4: Program address range
        ...
        # Step 5: Clear WP/RP (spec mandates WP=RP=0 before RamEnable)
    ...
    async def wait_empty(self, timeout=2000):
        """
        Wait for the DST encoder to drain.
        Strategy: first try polling bit 3 (spec position). If that bit never
        sets within 200 polls, fall back to a fixed 200-cycle drain wait and
        log a warning. This survives both correct and incorrect Empty bit positions.
        """
        for _ in range(min(200, timeout)):
            val = await self.apb.read(DST_REG["DST_CONTROL"])
            if (val >> DST_CTRL_EMPTY_BIT) & 1:
                return
            await ClockCycles(self.apb.dut.clk, 1)
        # Fallback: fixed drain wait (enough for any residual packets to flush)
        logger.warning("DSTDriver.wait_empty: Empty bit not seen — using fixed drain wait")
        await ClockCycles(self.apb.dut.clk, 200)

    async def wait_ram_empty(self, timeout=2000):
        """Same strategy as wait_empty but for the RAM sink."""
        for _ in range(min(200, timeout)):
            val = await self.apb.read(DST_REG["DST_RAM_CONTROL"])
            if (val >> DST_RAM_CTRL_EMPTY_BIT) & 1:
                return
            await ClockCycles(self.apb.dut.clk, 1)
        logger.warning("DSTDriver.wait_ram_empty: RamEmpty bit not seen — using fixed drain wait")
        await ClockCycles(self.apb.dut.clk, 200)

    async def read_wp(self):
        """Read the DST write pointer. WP is hardware-updated; add settle cycles."""
        await ClockCycles(self.apb.dut.clk, 4)   # let pending WP update propagate
    ...
    async def read_sram_word(self):
        """
        Read one 32-bit word from the DST SRAM data register.
        ...
        The APB read of DST_RAM_DATA itself triggers the RP-based read enable.
        After the APB transaction completes, wait RAM_DATA_READ_LATENCY_CYCLES
        extra clocks then re-read to get the valid data.
        Note: each APB read of the DATA register also auto-increments the RP.
        """
        await self.apb.read(DST_REG["DST_RAM_DATA"])   # trigger RP read
        await ClockCycles(self.apb.dut.clk, RAM_DATA_READ_LATENCY_CYCLES)
        return await self.apb.read(DST_REG["DST_RAM_DATA"])  # 2nd read = valid data

    async def full_init(self, start=0, limit=0x7FFF):
        """
        Full DST init per spec p.26.
        FIX 1: programs Trdstramvendorframelength (frame length must be non-zero
                for WP to advance and SRAM data to be written).
        FIX 2: enables the trace funnel (shared with NTrace at 0x4000).
        """
        await self.release_reset()
        await self.configure_sram(start=start, limit=limit)
        # Enable funnel (shared with NTrace)
        await self.apb.read_modify_write(
...
class NTraceDriver:
    ...
    async def configure_ram(self, start=0, limit=0x7FFF, stop_on_wrap=True,
                             frame_len=VENDOR_FRAME_LEN_64B):
        """
        Configure the NTrace RAM sink.

        CRITICAL FIX: programs Trramvendorframelength in the RAM_IMPL register
        BEFORE enabling the RAM.

        From dfd_trace_sink.sv:
          trntrFrameLength_ANY[9:0] = {TrCsrTrramimpl.Trramvendorframelength[3:0], 6'b0}
        When this field = 0, FrameLength = 0 → WP increments by 0 → no SRAM data.

        frame_len: bits[3:0] of the IMPL register (1 = 64-byte frames, default).
        """
        # Step 1: Activate RAM
        await self.apb.read_modify_write(
            NTR_REG["RAM_CONTROL"], set_bits=(1 << RAM_CTRL_ACTIVE_BIT))

        # Step 2: CRITICAL — program vendor frame length
        await self.apb.write(NTR_REG["RAM_IMPL"], frame_len & 0xF)
    ...
    async def wait_te_empty(self, timeout=2000):
        """
        Poll trTeEmpty (bit 3 of TE_CONTROL). Falls back to fixed drain wait
        if the bit never sets — the bit position may differ across builds.
        FIX: trTeEmpty at bit 3 is not confirmed in simulation (0x03000065
        shows bit 3 = 0 even after disable). Using 200-cycle fallback.
        """
        for _ in range(min(200, timeout)):
            val = await self.apb.read(NTR_REG["TE_CONTROL"])
            if (val >> TE_CTRL_EMPTY_BIT) & 1:
                return
            await ClockCycles(self.apb.dut.clk, 1)
        logger.warning("NTraceDriver.wait_te_empty: Empty bit not seen — using fixed drain wait")
        await ClockCycles(self.apb.dut.clk, 300)

    async def wait_funnel_empty(self, timeout=2000):
        """Same strategy for funnel empty."""
        for _ in range(min(200, timeout)):
            val = await self.apb.read(NTR_REG["FUNNEL_CONTROL"])
            if (val >> FUNNEL_CTRL_EMPTY_BIT) & 1:
                return
            await ClockCycles(self.apb.dut.clk, 1)
        logger.warning("NTraceDriver.wait_funnel_empty: FunnelEmpty not seen — using fixed drain")
        await ClockCycles(self.apb.dut.clk, 200)

    async def wait_ram_empty(self, timeout=2000):
        """Same strategy for RAM empty."""
        for _ in range(min(200, timeout)):
            val = await self.apb.read(NTR_REG["RAM_CONTROL"])
            if (val >> RAM_CTRL_EMPTY_BIT) & 1:
                return
            await ClockCycles(self.apb.dut.clk, 1)
        logger.warning("NTraceDriver.wait_ram_empty: RamEmpty not seen — using fixed drain")
        await ClockCycles(self.apb.dut.clk, 200)
    ...
    async def read_wp(self):
        """Read NTrace write pointer with settle cycles for HW update propagation."""
        await ClockCycles(self.apb.dut.clk, 4)
    ...
    async def read_sram_word(self):
        """
        Read one 32-bit word from the NTrace SRAM data register.
        Applies the 3-cycle read latency from dfd_apb2mmr.sv.
        Each read auto-increments the RP inside the hardware.
        """
        await self.apb.read(NTR_REG["RAM_DATA"])   # trigger RP read
        await ClockCycles(self.apb.dut.clk, RAM_DATA_READ_LATENCY_CYCLES)
        return await self.apb.read(NTR_REG["RAM_DATA"])
