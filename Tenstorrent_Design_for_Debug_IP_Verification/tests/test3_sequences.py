...
async def L3_SEQ_003_dst_correct_shutdown_order(dut):
    ...
    ctrl = await apb.read(DST_REG["DST_CONTROL"])
    if (ctrl >> DST_CTRL_EMPTY_BIT) & 1:
        log.info("L3_SEQ_003: trDstEmpty (bit 3) asserted ✓")
    else:
        log.warning(f"L3_SEQ_003: trDstEmpty bit 3 not set in 0x{ctrl:08X}. "
                    "The Empty bit position may differ in this build. "
                    "Drain is handled by wait_empty() fallback. Non-fatal.")
    ...
    ram_ctrl = await apb.read(DST_REG["DST_RAM_CONTROL"])
    if (ram_ctrl >> DST_RAM_CTRL_EMPTY_BIT) & 1:
        log.info("L3_SEQ_003: trDstRamEmpty asserted ✓")
    else:
        log.warning(f"L3_SEQ_003: trDstRamEmpty bit {DST_RAM_CTRL_EMPTY_BIT} not set "
                    f"in 0x{ram_ctrl:08X}. Empty bit position may differ. Non-fatal.")
...
async def L3_SEQ_004_ntr_correct_shutdown_order(dut):
    ...
    ctrl = await apb.read(NTR_REG["TE_CONTROL"])
    if (ctrl >> TE_CTRL_EMPTY_BIT) & 1:
        log.info("L3_SEQ_004: trTeEmpty asserted ✓")
    else:
        log.warning(f"L3_SEQ_004: trTeEmpty bit {TE_CTRL_EMPTY_BIT} not set in "
                    f"0x{ctrl:08X}. Drain handled by fallback wait. Non-fatal.")
    ...
    if (ram_ctrl >> RAM_CTRL_EMPTY_BIT) & 1:
        log.info("L3_SEQ_004: trRamEmpty asserted ✓")
    else:
        log.warning(f"L3_SEQ_004: trRamEmpty bit {RAM_CTRL_EMPTY_BIT} not set in "
                    f"0x{ram_ctrl:08X}. Non-fatal.")
...
async def L3_SEQ_010_te_empty_gates_data(dut):
    ...
    if wp_later != wp_at_empty:
        log.warning(f"L3_SEQ_010: WP changed after trTeEmpty: "
                    f"0x{wp_at_empty:X} → 0x{wp_later:X}. "
                    "Empty bit position may be wrong (data still in flight). Non-fatal.")
    else:
        log.info("L3_SEQ_010: WP stable after trTeEmpty ✓")
...
async def L3_SEQ_011_funnel_empty_gates_ram_stop(dut):
    ...
    if wp_later != wp_at_funnel_empty:
        log.warning(f"L3_SEQ_011: WP moved after FunnelEmpty: "
                    f"0x{wp_at_funnel_empty:X} → 0x{wp_later:X}. Non-fatal.")
    else:
        log.info("L3_SEQ_011: WP stable after FunnelEmpty ✓")
